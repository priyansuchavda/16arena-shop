    ...
    <div className="relative rounded-2xl overflow-hidden">
      <div className="absolute inset-0 rounded-2xl" style={{ padding: 1 }}>
        <div
          ...
          style={{
            background: "linear-gradient(135deg, rgba(255,255,255,0.18) 0%, rgba(255,255,255,0.04) 75%)",
          }}
        />
      </div>
      <div className="relative rounded-2xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-sm">
        {children}
      </div>
    </div>
  );
...
function HeroCard({ order }: { order: ShopOrder }) {
  ...
  return (
    <div
      ...
      style={{ aspectRatio: "1.586/1", background: "linear-gradient(135deg, #2a2a2a 0%, #404040 100%)" }}
    >
      {/* Glare overlay */}
      <div className="absolute inset-0 opacity-30" style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.25) 0%, transparent 50%)" }} />

      {/* Brand logo top-left */}
      <div className="absolute top-4 left-4 flex items-center gap-2.5">
        {logoUrl ? (
          <div className="relative w-10 h-10 rounded-xl overflow-hidden bg-black/20 border border-white/10">
